...
import timefold.ui.backend.bedallocation.domain.*;
...
public class DemoDataGenerator {
    ...
    private List<Room> generateRooms(int size, List<Department> departments, Random random) {
        ...
        applyRandomValue(size, rooms, r -> r.getGenderLimitation() == null,
                r -> r.setGenderLimitation(GenderLimitation.SAME_GENDER), random);
    ...
    private void generatePatients(List<Stay> stays, Random random) {
        // 50% MALE - 50% FEMALE
        applyRandomValue((int) (stays.size() * 0.5), stays, p -> p.getPatientGender() == null, p -> p.setPatientGender(Gender.MALE), random);
        applyRandomValue((int) (stays.size() * 0.5), stays, p -> p.getPatientGender() == null, p -> p.setPatientGender(Gender.FEMALE), random);
        stays.stream().filter(p -> p.getPatientGender() == null).forEach(p -> p.setPatientGender(Gender.MALE));
